...
```
O servidor sobe em `http://10.111.9.64:3000` (ou na porta definida em `PORT`).
...
Se o back-end estiver em outro endereço (ex: IP da intranet do laboratório, como `10.111.9.64`), ajuste em `assets/js/config.js`:
```js
API_BASE: "http://10.111.9.64:3000",
```
